...
do
    ...
    if [ $IS_LABEL = "No" ];then
    echo "IS_LABEL:No"
    python3 createThinPatch.py ${image_path} ${MODEL_PATH} ${save_path} --label_patch_size ${LABEL_PATCH_SIZE} --plane_size ${PLANE_SIZE} --overlap ${OVERLAP} --num_down ${NUM_DOWN} 
    else
    echo "IS_LABEL:Yes"
    python3 createThinPatch.py ${image_path} ${MODEL_PATH} ${save_path} --label_patch_size ${LABEL_PATCH_SIZE} --plane_size ${PLANE_SIZE} --overlap ${OVERLAP} --num_down ${NUM_DOWN} --is_label
    fi
